...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
       ...
       # a. Set the string variable model_label to be the string that's 
       #    returned from using the classifier function
       model_label = classifier(images_dir + "/" + key, model)

       # b. Process the model_label to convert all characters to lowercase 
       #    and strip any whitespace characters from the ends
       model_label = model_label.lower().strip()

       # defines truth as pet image label 
       truth = results_dic[key][0]

       # c. If the pet image label is found within the classifier label list of terms 
       ...
       if truth in model_label:
           ...
           # d. If the pet image label is not found in the classifier label, 
           #    then it's added as NOT a match (0) using extend list function
